...
const commands = {
  async start() {
    ...
    const grafanaCmd =
      'docker run -d --name grafana -p 3002:3000 -e "GF_SECURITY_ADMIN_PASSWORD=admin" grafana/grafana:latest';
  ...
  async status() {
    ...
    const services = [
      ...
      { port: 3002, name: 'Grafana', url: 'http://localhost:3002' },
    ];
